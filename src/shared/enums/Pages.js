export const Pages = {
    Home: {
        id: 0,
        name: 'home',
        path: '',
        daysAndTimes: []
    },
    TrabalhoReligioso: {
        id: 1,
        name: 'Trabalho Religioso',
        path: 'trabalho-religioso',
        daysAndTimes: []
    },
    ReuniaoPublica: {
        id: 2,
        name: 'Reunião Pública',
        path: 'reuniao-publica',
        daysAndTimes: ['Segunda-feira às 20:00']
    },
    RecepcaoEAtendimentoFraterno: {
        id: 3,
        name: 'Atendimento Fraterno',
        path: 'recepcao-atendimento-fraterno',
        daysAndTimes: ['Segunda-feira às 20:00', 'Quarta-feira às 20:00']
    },
    SosPreces: {
        id: 4,
        name: 'SOS Preces',
        path: 'sos-preces',
        daysAndTimes: ['Atendimento diariamente pelo telefone: (31) 3334-9700']
    },
    ProgramaRenascer: {
        id: 5,
        name: 'Programa Renascer',
        path: 'programa-renascer',
        daysAndTimes: ['Quinta-feira às 20:00']
    },
    EvangelizacaoInfantil: {
        id: 6,
        name: 'Evangelização Infantil',
        path: 'evangelizacao-infantil',
        daysAndTimes: ['Segunda-feira às 20:00']
    },
    CampanhaDoQuiloEDoacaoDeCestasBasicas: {
        id: 7,
        name: 'Campanha do Quilo',
        path: 'campanha-quilo-doacao-cestas-basicas',
        daysAndTimes: ['Sábado às 8:45', 'Domingo às 8:45']
    },
    DistribuicaoDeSopaELanche: {
        id: 8,
        name: 'Distribuição de Lanche',
        path: 'distribuicao-sopa-lanche',
        daysAndTimes: []
    },
    EducacaoMediunica: {
        id: 9,
        name: 'Educação Mediúnica',
        path: 'educacao-mediunica',
        daysAndTimes: ['Quinta-feira às 19:30']
    },
    Livraria: {
        id: 10,
        name: 'Livraria',
        path: 'livraria',
        daysAndTimes: ['Segunda-feira de 20:00 às 21:00']
    }
};